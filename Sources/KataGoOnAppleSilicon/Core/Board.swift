...
public class Board {
	...
	public func isLegalMove(at point: Point, stone _: Stone) -> Bool {
		guard point.isValid, stones[point.y][point.x] == .empty else { return false }
		return true
	}
	...
	private func calculateAreaForPla(pla: Stone, safeBigTerritories: Bool, unsafeBigTerritories: Bool, isMultiStoneSuicideLegal _: Bool, result: inout [[Stone?]]) {
		let opp: Stone = (pla == .black) ? .white : .black
		var groupHeads: [Point: Point] = [:]
		...
		var vitalForPlaHeads: [[Point]] = []
		var numInternalSpacesMax2: [Int] = []
