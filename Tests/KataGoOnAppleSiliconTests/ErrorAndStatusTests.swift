...
import Testing
@Test func testModelStatusReportLoadFailed() throws {
	struct TestError: Error {}
	ModelStatus.reportModelLoadFailed(name: "TestModel", error: TestError())
}
@Test func testModelStatusReportModelLoaded() throws {
	ModelStatus.reportModelLoaded(name: "TestModel", time: 1.23)
}
@Test func testModelStatusReportInferenceCompleted() throws {
	ModelStatus.reportInferenceCompleted(time: 0.45, policyCount: 361, value: 0.5)
}
@Test func testModelStatusReportInferenceFailed() throws {
	struct TestError: Error {}
	ModelStatus.reportInferenceFailed(error: TestError())
}
@Test func testKataGoErrorDescriptions() throws {
...
@Test func testMoveStaticPass() throws {
	let move = Move.pass(player: .black)
	#expect(move.location == nil)
	#expect(move.player == .black)
	#expect(move.isPass)
}
@Test func testMoveIsPassProperty() throws {
